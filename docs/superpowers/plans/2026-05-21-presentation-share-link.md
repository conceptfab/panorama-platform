...
W `interface PanoViewerProps` (linia ~17) dodaj po `projectId?: string;`:
...
W `export function PanoViewer({ ... })` (linia ~24) dodaj `publicMode` i `shareToken`:
...
W `useEffect` statystyk (linia ~300) zamień oba `fetch('/api/stats', ...)` na `fetch(statsEndpoint, ...)` i dodaj `statsEndpoint` do tablicy zależności. Efekt końcowy:
...
W `handleScreenshot` (linia ~485) zamień `fetch('/api/stats', ...)` na `fetch(statsEndpoint, ...)` i dodaj `statsEndpoint` do zależności `useCallback` (linia ~506): `}, [config.projectName, projectId, statsEndpoint]);`.

- [ ] **Step 6: Ukryj link „powrót do galerii" w trybie publicznym**

Zamień blok `<Link href="/gallery" ...> ... </Link>` (linia ~643) na warunkowy:
...
Następnie w JSX, **po** zamykającym `</Card>` głównej karty „Edycja projektu" a **przed** zamykającym `</div>` najbardziej zewnętrznego kontenera, dodaj:
